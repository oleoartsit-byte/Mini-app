import { PrismaClient, TutorialType, TutorialStatus } from '@prisma/client';
...
Suitable for all new users!`,
  ...
  const published = await prisma.tutorial.count({ where: { status: TutorialStatus.PUBLISHED } });
